...
from blockstream import get_height, get_mempool_stats, get_transaction_status, get_mempoolfees
...
bot_commands = ["<b>/all</b> \t\t - List Open Offers. \n",
                ...
                "<b>/get_block_height</b> \t\t - get bitcoin block height\n",
                "<b>/gettx </b> \t\t- < txid > get L1 transaction status \n",
                "<b>/get_mempool_stats</b> \t\t- L1 mempool stats \n",
                "<b>/fees</b> \t\t- get fee estimates \n"
...
async def get_hkdsat(client, chatid):
    homedir = os.getcwd()
    satpath = homedir + '/images/sats.png'
    await client.send_file(chatid, satpath)
    await client.send_message(chatid, 'source: sats.bitcoin.org.hk')


async def get_whatthefee(client, chat_id):
    homedir = os.getcwd()
    wtf_path = homedir + '/images/wtf.png'
    
    if not os.path.exists(wtf_path):
        logging.info(f'path does not exist: {wtf_path}')
        return
    else:
        logging.info('getting whatthefee ')
        # use chat_id to send to sender in DM, or group,
        # wherever the event originated
        # use in conjunction with fee snapshot
...
async def handler(event):
    ...
    if '/get_block_height' in rawtext:
        msg = get_height()
        await event.reply(msg)
    ...
    if '/gettx' in rawtext:
        ...
        if len(content) == 1:
            await event.reply('Usage: /gettx < transaction id >')
        else:
            msg = get_transaction_status(rawtext)
            await event.reply(msg) 
    ...
    if '/hkdsat' in rawtext: 
        ...
        await get_hkdsat(client, chat_id)

    if '/fees' in rawtext:
        res = get_mempoolfees()
        msg = 'Fees recommended by mempool.space\n\n'
        for key,value in res.items():
            msg += key + " : " + str(value) + "\n" 
        await event.reply(msg)
        chat_id = event.chat_id
        await get_whatthefee(client, chat_id)
